...
function Signup() {
  ...
  const handleGoogleSuccess = async ({ credential }) => {
    try {
      ...
      if (res.status === 201) {
        ...
        notify('Erro', 'Falha ao registar com Google. Tente novamente.');
      }
    } catch (error) {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        notify("Sucesso", "Conta criada com sucesso, clica em iniciar sessão para entrares.")
      } else if (res.status === 404) {
        setError('Dados inválidos. Verifique o email e a palavra-passe.');
      }
      else if (res.status === 409) {
        setError('Já existe um utilizador com este username ou e-mail, por favor insere um novo email/username.');
          ...
          <Card className="w-full max-w-sm bg-gray-900 border-gray-800 shadow-xl rounded-2xl">
            <CardContent className="p-8">
              ...
              <h3 className="text-xl font-extrabold text-white leading-tight tracking-tight">
                ...
                <span className="text-green-400">daquilo que eles te devem.</span>
              </h3>
              ...
              {/* Criar Conta */}
              <p className="text-white font-bold text-center mt-8 mb-6 text-xl">Criar Conta</p>
              ...
              <div className="flex flex-col gap-3 mb-4">
                ...
                <p className="text-gray-400 text-xs">
                  Já tens uma conta?{' '}
                  <a href="/signin" className="text-blue-400">Iniciar Sessão</a>
                </p>
              </div>
              ...
              <div className="flex justify-center">
                <GoogleLogin
                  ...
                  onError={() => notify('Erro', 'Falha ao registar com Google.')}
                  theme="outline"
                  shape="pill"
                  size="large"
                  text="signup_with"
