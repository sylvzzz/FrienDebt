  ...
  try {
    ...
    db.query('SELECT * FROM utilizadores WHERE email = ?', [email], (err, results) => {
      ...
      if (results.length > 0) {
        ...
        req.session.user = { 
          id: results[0].id,       // 🔹 garante que o id é guardado
          nome: results[0].nome, 
          email: results[0].email 
        };
        res.cookie('userEmail', results[0].email, { httpOnly: false });
        ...
        db.query(
          ...
          (err, result) => {
            ...
            // 🔹 guarda o id do novo utilizador na sessão
            req.session.user = { 
              id: result.insertId,
              nome: name, 
              email 
            ...
            res.cookie('userEmail', email, { httpOnly: false });
...
db.query(query, [email, email, email, email, email, email, email, email], (err, results) => {
...
app.post('/marcar', (req, res) => {
  ...
  db.query(queryPagador, [pagador], (err, rowsPagador) => {
    ...
    if (rowsPagador.length === 0) return res.status(404).send('Pagador não encontrado');
    ...
    db.query(queryCobrador, [cobrador], (err, rowsCobrador) => {
      if (err) return res.status(500).send('Erro no servidor');
      if (rowsCobrador.length === 0) return res.status(404).send('Cobrador não encontrado');
      ...
      `;
      db.query(queryAmigos, [idPagador, idCobrador, idCobrador, idPagador], (err, rowsAmigos) => {
        if (err) return res.status(500).send('Erro no servidor');
        if (rowsAmigos.length === 0) return res.status(403).send('Não tens ', pagador," adicionado/a como amigo. Só podes adicionar dívidas a utilizadores de que és amigo.");

        const queryInsert = `
          INSERT INTO dividas (id_pagar, id_cobrador, valor, dat, descricao, paid)
          VALUES (?, ?, ?, ?, ?, ?)
        `;
        db.query(queryInsert, [idPagador, idCobrador, valor, data, descricao, 0], (err) => {
          if (err) return res.status(500).send('Erro no servidor');
          res.status(201).send('Dívida criada com sucesso');
          console.log(cobrador, " posted a new Debt: ", pagador, " owes ", valor, "€ for ", descricao);
        });
      });
    });
  });
...
app.post('/remover-amigo', (req, res) => {
  ...
  db.query(queryAmigo, [nome_amigo], (err, rows) => {
    ...
    const queryRemover = `
      ...
      WHERE (id_x = ? AND id_y = ?) OR (id_x = ? AND id_y = ?)
    `;
    db.query(queryRemover, [idUser, idAmigo, idAmigo, idUser], (err, result) => {
      if (err) {
...
app.get('/utilizadores/pesquisar', (req, res) => {
  ...
  const query = `
    SELECT u.nome, u.email,
      EXISTS(
        SELECT 1 FROM conns 
        WHERE id_x = (SELECT id FROM utilizadores WHERE email = ?)
        AND id_y = u.id
      ) AS ja_amigo
    FROM utilizadores u
    WHERE u.nome LIKE ? AND u.email != ?
  `;
  db.query(query, [email, `%${nome}%`, email], (err, results) => {
...
app.post('/updateuser', (req, res) => {
  ...
  db.query('SELECT * FROM utilizadores WHERE nome = ? AND email != ?', [nome, email], (err, results) => {
    if (err) return res.status(500).send('Erro no servidor.');
    if (results.length > 0) return res.sendStatus(409);

    db.query('UPDATE utilizadores SET nome = ? WHERE email = ?', [nome, email], (err) => {
